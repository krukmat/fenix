...
import { catalog, type ResolvedIds } from './admin-screenshots/catalog';
...
async function resolveIds(goUrl: string, token: string, workspaceId: string): Promise<ResolvedIds> {
  ...
  const auditResp = await fetch(`${goUrl}/api/v1/audit/events?limit=1`, { headers });
...
async function main(): Promise<void> {
  ...
  process.stdout.write(`[admin-screenshots] IDs resolved — audit: ${resolved.firstAuditEventId}\n`);
