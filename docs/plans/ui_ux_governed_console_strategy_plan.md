...
| `UIX-22A` | Sales brief contract unblock: per-item evidence citation linkage + run/usage reference | dev (go/mobile contract) | none | UIX-22 (partial — evidence citations, usage footer) | standard |
...
**Status update (2026-07-05)**: `UIX-10A` completed and unblocked `UIX-10`. `UIX-10` is now completed: `EvidenceCard` renders the currently truthful per-source trust fields (`retrieval_method`, `pii_redacted`, stable mobile `timestamp`, and `knowledge_item_id`) in expanded state without fabricating provenance or stale markers that the backend does not yet assert per source. During `UIX-11` execution we found a second contract gap: the copilot stream did not expose truthful `confidence`/`warnings` metadata to mobile and the mobile SSE parser discarded terminal metadata that already carried `answer_type`/`abstention_reason`. `UIX-11A` is now completed: the Go copilot stream emits truthful evidence-pack trust metadata and preserves done-event outcome metadata, while the mobile SSE parser and message shaping retain those fields for downstream rendering. `UIX-11` is now completed: `CopilotPanel` renders the inline trust unit with truthful confidence tier badge, warnings row, and designed abstention panel while preserving streaming semantics, `testID`s, evidence reuse, and `onSupportTrigger`. `UIX-12` is now completed: approval cards render the governed five-state FSM stepper (`pending`, `approved`, `rejected`, `expired`, `cancelled`), normalize legacy `denied` to `rejected` for display only, and show a compact policy explanation block only when machine-readable policy metadata is already present in `approval.payload`, without changing approval action wiring or existing `testID`s. `UIX-13` is now completed: the mobile approval write surface keeps the existing backend decision authority and transport semantics while adding an optional approval comment dialog, per-approval terminal/conflict feedback in the inbox, terminal `ApprovalPath` rendering after successful decisions, and designed expired/already-decided states with an audit-trail affordance when trace metadata is present. `UIX-14` is now completed: the activity run detail screen has been refactored into a modular Run Inspector that surfaces truthful highlights, evidence-pack metadata when present, tool activity, awaiting-approval linkage, handoff payload retrieval, reasoning trace, outcome payload, and per-run usage, while routing trace visibility to the existing audit screen without fabricating the filtered deep-link reserved for `UIX-15`. `UIX-15` is now completed: `trace_id` now flows end-to-end from the Run Inspector's "Open audit trail" action and the approval decision feedback's "View audit trail" action into the governance audit screen, which seeds its filter state from the route param on mount (normalizing an array param to its first value) and falls back to the existing unfiltered behavior when no trace_id is present; `AuditEventCard` also renders its own trace_id as a tappable mono link into the same filtered view, staying neutral, non-interactive text when the field is absent. No backend contract, approval decision, or audit persistence semantics changed — this is client-side routing over the `trace_id` filter UIX-15A already added to the audit query contract. `UIX-21` is now completed: the Inbox screen reframes the existing interleaved approvals/handoffs/signals/rejected feed into five named decision-queue groups ("Awaiting your approval", "Recently decided", "Handoffs waiting", "Signals to review", "Denied runs") with count eyebrows, outcome/expiry accent stripes reusing `ApprovalPath`/`semantic.ts` colors, and a "No decisions waiting" designed empty state, over the same `useInbox` payload and sort semantics. Peer code review surfaced and fixed five correctness gaps beyond the original grouping scope: the approval accent color now reuses `ApprovalPath`'s own step-color map instead of a status map missing `ApprovalStatus` entries; an "already decided" conflict now renders a neutral, non-fabricated "Closed" state (no buttons, no FSM stepper, no stale countdown) instead of still looking actionable; decided approvals sort by actual decision recency (`decidedAt`/refreshed `updatedAt`) instead of a stale timestamp; a 60s screen-level tick re-groups an approval that crosses expiry while the screen stays open (mirroring `ApprovalCard`'s own countdown refresh); and a new `reconcileFeedbackWithServer` helper keeps deterministically-known local outcomes (own success, or a locally-verifiable state like "expired") visible across refetch while dropping only unresolved guesses ("already decided") once the server stops corroborating them. `UIX-22` is now completed: the sales account and deal detail screens embed the upgraded `CopilotPanel` scoped to their entity, the deal screen renders the new `DealStagePath` stepper (generalized from `ApprovalPath`, unknown stages render neutrally) in place of plain stage text, and `SalesBriefContent` renders a shared `ConfidenceBadge` (top-level and per-next-best-action) plus a designed abstention panel, reusing fields the `SalesBrief` contract already exposes. `ConfidenceBadge` and `AbstentionPanel` were extracted out of `CopilotPanel` into standalone components (`mobile/src/components/copilot/{ConfidenceBadge,AbstentionPanel}.tsx`) to make them genuinely shared rather than duplicated, since neither previously existed outside `CopilotPanel`'s private inline functions. Per-section evidence citations and the usage footer remain deferred to `UIX-22A` as planned — the evidence-pack-level block is unchanged and no contract fields were fabricated. `UIX-31` is now completed: the support case detail screen was reframed as supporting context reached from a decision, with a highlights block (subject, status chip, account/contact/owner), a truthful `CaseStatusPath` that only orders backend-known statuses (`open`, `in_progress`, `waiting`, `resolved`, `closed`) and falls back neutrally for any other raw status, a timeline tab that merges `GET /timeline/case/{id}` output with activities and case-filtered notes into one newest-first feed, and an inline `CopilotPanel` scoped to the case so support runs now launch in-context rather than via a separate copilot route. The evidence tab remains intentionally absent: no knowledge-linked-to-case contract exists independently of a run, so the screen documents that gap by omission rather than fabricating unsupported content.
...
- **Contract gap (2026-07-05)**: the current `SalesBrief` contract (`mobile/src/services/api.types.ts`) has no `evidence_ids`/citation field on `risks`/`nextBestActions`, and no `run_id`/usage fields — so true per-section evidence citations and a Run-Inspector-identical usage footer cannot be built without fabricating data. `UIX-22A` tracks the backend contract unblock; until it lands, `UIX-22` implements confidence rendering (shared `ConfidenceBadge`, per-action confidence from the fields that already exist) and the designed abstention state, and renders the existing evidence-pack-level block unchanged, explicitly deferring per-section citations and the usage footer rather than inventing them client-side.
- **BDD impact**: none new (`uc-s1`/`uc-s1-sales-copilot-mobile-smoke`). **QA**: `bash scripts/qa-mobile-prepush.sh`.
- **Model**: `OpenAI: gpt-5.4 | Anthropic: claude-sonnet-4-6` · Effort: Medium

### UIX-22A — Sales brief contract unblock (evidence citation linkage + run/usage reference)

- **Files**: `internal/domain/copilot/*` (sales-brief handler), `internal/domain/sales/*` if brief generation lives there; `mobile/src/services/api.types.ts` (extend `SalesBrief`/`SalesBriefAction` typing to match); tests
- **Scope**: Extend the `POST /api/v1/copilot/sales-brief` response so `risks`/`nextBestActions` carry truthful per-item `evidence_ids` (or equivalent citation linkage back into `evidencePack.sources`), and so the response carries a `run_id` (or equivalent) plus enough usage data (or a queryable run reference) for the mobile Run Inspector's usage pattern to apply. Do not fabricate citations or usage figures that the backend cannot assert truthfully; if full per-item citation isn't feasible, expose the truthful subset and document the residual gap (mirrors `UIX-10A`'s "expose only supported fields" precedent).
- **BDD impact**: none expected (contract-enablement only; user-visible behavior lands in `UIX-22`). **QA**: `bash scripts/qa-go-prepush.sh` plus `bash scripts/qa-mobile-prepush.sh`.
- **Model**: `OpenAI: gpt-5.4 | Anthropic: claude-sonnet-4-6` · Effort: Medium
...
4. Support cases and sales accounts/deals are reached *from decisions*, each with highlights → path → timeline and an embedded trust unit that shows confidence and renders abstention as a designed state (UIX-22, UIX-31). Per-section evidence citation and a cost/usage footer on these record screens are gated on `UIX-22A` (sales brief contract unblock — the current `SalesBrief` contract has no per-item citation linkage and no run/usage reference); until `UIX-22A` lands, the trust unit on these screens renders the existing evidence-pack-level block and omits cost, rather than fabricating either.
