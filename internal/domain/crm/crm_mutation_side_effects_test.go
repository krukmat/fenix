...
func TestLeadDealCaseNoteServices_EmitAuditAndTimeline(t *testing.T) {
	...
	assertTimelineCount(t, db, wsID, "case", caseTicket.ID, "created", 1)
	assertTimelineCount(t, db, wsID, "case", caseTicket.ID, "updated", 1)
	assertTimelineCount(t, db, wsID, "case", caseTicket.ID, "deleted", 1)
