...
.PHONY: all test build run lint fmt complexity race-stability coverage-gate coverage-tdd check migrate-up migrate-down migrate-create migrate-version sqlc-generate docker-build docker-run e2e clean db-shell
...
# Coverage gate over full repository profile generated by `make test`
# Ignores package-level 0.0% lines; uses global total from coverprofile.
# Stage 1 uplift (Task 2.6 coverage): 30 -> 35
COVERAGE_MIN?=35
coverage-gate:
	@echo "Checking global coverage threshold ($(COVERAGE_MIN)%)..."
	@total=$$(go tool cover -func=coverage.out | awk '/^total:/ {gsub("%", "", $$3); print $$3}'); \
	if [ -z "$$total" ]; then \
		echo "FAILED: could not read total coverage from coverage.out"; \
		exit 1; \
	fi; \
...
ci: fmt complexity lint test race-stability coverage-gate coverage-tdd build
