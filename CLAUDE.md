...
- Before starting work on each discrete task, present the task card to the user first, before reading, editing, or running task-specific commands except for minimal inspection needed to identify the next task. The task card must include:
  ...
  - `Peer readiness review approval: reviewer=<reviewer>; artifact=<artifact path>; status=PASS` — required for every task, with no exceptions. This field confirms the task file and governing docs passed independent peer review before task-card presentation. The `artifact` value is the proof file; the `status` value is the approval state. Only `status=PASS` unblocks presentation. Any other verdict — `fail`, `blocked`, reviewer unavailable, or reviewer unauthenticated — halts presentation. There is no waiver, no omission, and no acceptable terminal `BLOCKED` state: the caller must revise the task file or restore reviewer availability and re-run the gate, then present the task card only once a `PASS` verdict exists.
...
- When closing a task, report the outcome with:
  ...
  - `Peer code review approval: reviewer=<reviewer>; artifact=<artifact path>; status=PASS` — required for every development task, with no exceptions. The `artifact` value is the proof file; the `status` value is the approval state. Only `status=PASS` unblocks closure. Any other verdict — `fail`, `blocked`, reviewer unavailable, or reviewer unauthenticated — halts closure. There is no waiver, no omission, and no acceptable terminal `BLOCKED` state: the caller must revise the code or restore reviewer availability and re-run the gate, then close only once a `PASS` verdict exists. Peer review does not replace RRI/HITL human approval.
