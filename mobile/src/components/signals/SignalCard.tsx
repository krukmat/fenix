// Task Mobile P1.3 — UC-A5/B4: Signal card for list display
...
function confidenceColor(confidence: number): string {
  ...
  if (confidence >= 0.5) return '#e65100'; // amber
  return '#757575'; // gray
}
    ...
    <>
        ...
        <Card.Content>
          <View style={styles.header}>
            <View style={styles.titleRow}>
              <Text variant="labelLarge" testID={`${testIDPrefix}-type`}>
                {signal.signal_type}
              </Text>
              <Chip
                compact
            ...
            <IconButton
              icon="close"
              size={18}
              testID={`${testIDPrefix}-dismiss-btn`}
          ...
          <Text
            variant="bodySmall"
            style={[styles.entity, { color: theme.colors.onSurfaceVariant }]}
      ...
      <Portal>
        <Dialog
          ...
          testID={`${testIDPrefix}-dismiss-dialog`}
        >
          ...
          <Dialog.Actions>
            <Button onPress={() => setConfirmVisible(false)} testID={`${testIDPrefix}-dismiss-cancel`}>
              Cancel
            </Button>
            <Button onPress={handleDismissConfirm} testID={`${testIDPrefix}-dismiss-confirm`}>
              Dismiss
            </Button>
          </Dialog.Actions>
        </Dialog>
